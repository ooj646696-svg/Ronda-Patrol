...
const geocodingCache = new Map<string, GeocodedAddress>();

// Track pending requests to avoid duplicates
const pendingRequests = new Map<string, Promise<GeocodedAddress | null>>();
...
async function fetchReverseGeocode(
  ...
  try {
    ...
    const response = await fetch(url, {
      headers: {
        'User-Agent': 'RONDA-Patrol-App/1.0',
      },
    });
 ...
 */
export function formatLocationName(address: GeocodedAddress | null): string {
 ...
 */
export function getShortLocationName(address: GeocodedAddress | null): string {
